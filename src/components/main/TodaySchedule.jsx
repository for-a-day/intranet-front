...
const TodaySchedule = () => {
  ...
  useEffect(() => {
    const fetchTodayActivities = async () => {
      try {
        const response = await axios.get("http://localhost:9000/app/schedule/32");
        ...
        const todayActivities = schedules
          .filter(
            (schedule) =>
              new Date(schedule.startDate).toISOString().split("T")[0] === today
          )
          .map((schedule) => ({
        ...
        console.log("today", todayActivities);
